				...
				echo '<li>'.
					$this->Default2->button(
						...
						array(
							...
							'enabled' => ( $ajout_possible && $this->Permissions->check( 'reorientationseps93', 'add' ) )
						)
					).
				' </li>';
			?>
			...
			<?php
				echo '<li>'.
					$this->Xhtml->addLink(
						...
						$ajout_possible && $this->Permissions->check( 'proposorientationscovs58', 'add' )
					).
				' </li>';
			?>
		</ul>
				...
				if( Configure::read( 'Cg.departement' ) != 93 ){
					echo '<li>'.
						$this->Xhtml->addLink(
							...
							!$force_edit && $ajout_possible && $this->Permissions->check( 'orientsstructs', 'add' )
						).
					' </li>';
				}
				else {
					echo '<li>'.
						$this->Default2->button(
							...
							array(
								'label' => 'Demander une réorientation',
								'enabled' => ( !$force_edit && $ajout_possible && $this->Permissions->check( 'orientsstructs', 'add' ) )
							)
						).
					' </li>';
				}
						...
						if( Configure::read( 'Cg.departement' ) == 66 ) {
							array_push(
								...
								$this->Default2->button(
									...
									array(
										'enabled' => (
											$this->Permissions->check( 'orientsstructs', 'edit' ) && ( $orientstruct['Orientstruct']['rgorient'] == $rgorient_max )
								...
								$this->Default2->button(
									...
									array(
										'enabled' => (
											( $this->Permissions->check( 'orientsstructs', 'impression' ) == 1 )
								...
								$this->Default2->button(
									...
									array(
										'enabled' => (
											( $this->Permissions->check( 'orientsstructs', 'printChangementReferent' ) == 1 )
								...
								$this->Default2->button(
									...
									array(
										'enabled' => (
											$this->Permissions->check( 'orientsstructs', 'delete' )
							...
							array_push(
								...
								$this->Xhtml->printLink(
									...
									array( 'controller' => 'orientsstructs', 'action' => 'impression', $orientstruct['Orientstruct']['id'] ),
									$this->Permissions->check( 'orientsstructs', 'impression' ) && $orientstruct['Orientstruct']['imprime']
								),
