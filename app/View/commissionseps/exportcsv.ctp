<?php
	...
	$csv->addRow( array( 'Nom/Prénom allocataire', 'N° CAF', 'Date de naissance', 'Commune de l\'allocataire', 'Structure référente', 'Date de création du dossier', 'Thème du dossier', 'Etat du dossier d\'EP', 'Date de proposition validée par la COV' ) );
	...
	foreach( $dossierseps as $dossierep ) {
		...
		$row = array(
			...
			$locale->date( 'Date::short', Set::classicExtract( $dossierep, 'Dossierep.Personne.dtnai' ) ),
			...
			$locale->date( 'Date::short', Set::classicExtract( $dossierep, 'Dossierep.created') ),
			...
			$locale->date( 'Date::short', Set::classicExtract( $dossierep, 'Dossierep.Nonorientationproep58.Decisionpropononorientationprocov58.Passagecov58.Cov58.datecommission') )
		);
		$csv->addRow($row);
	}
