...
// debug( $dsp);
			if( empty( $dsp['Dsp']['id'] ) ) {
				...
				if( $this->Permissions->check( 'dsps', 'add' ) ) {
				...
				if( $this->Permissions->check( 'dsps', 'edit' ) && ( (isset($rev)) && (!$rev) /*|| ( $this->action == 'view_revs' )*/ ) ) {
					echo '<ul class="actionMenu">
				...
				if( $this->Permissions->check( 'dsps', 'revertTo' ) && ( $this->action == 'view_revs' ) ) {
					echo '<ul class="actionMenu">
