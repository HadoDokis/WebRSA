			...
			.'</li><li>'.$this->Xhtml->printCohorteLink(
				...
				Hash::merge(
					array(
						...
						'action'     => 'cohorte_imprimer_impressions',
					),
					Hash::flatten( $this->request->data, '__' )
				)
				, ( $this->Permissions->check( $controller, 'cohorte_imprimer_impressions' ) && $count > 0 )
				, 'Voulez vous imprimer les '.($count-1).' dossiers PCGs ?'
				, 'popup_Dossierpcg66'
			).'</li><li>
