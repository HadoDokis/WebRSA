...
<script type="text/javascript">
	...
	<?php if( $personnedecisionmodifiable ):?>
		<fieldset>
			...
			<?php if( $this->Permissions->check( 'personnespcgs66', 'add' ) ):?>
				<ul class="actionMenu">
					<?php
						echo '<li class="add">'.$this->Xhtml->addLink(
							'Ajouter une personne',
							array( 'controller' => 'personnespcgs66', 'action' => 'add', $dossierpcg66_id ),
							( $etatdossierpcg != 'attaffect' || $this->Permissions->check( 'personnespcgs66', 'add' ) != "1" )
						).' </li>';
					?>
				</ul>
			<?php endif;?>
								...
								echo $this->Xhtml->tableCells(
									array(
										...
										$this->Xhtml->treatmentLink(
											...
											$this->Permissions->check( 'traitementspcgs66', 'index' )
										),
										$this->Xhtml->deleteLink(
											'Supprimer la personne',
											array( 'controller' => 'personnespcgs66', 'action' => 'delete', $personnepcg66['Personnepcg66']['id'] ),
											$this->Permissions->check( 'personnespcgs66', 'delete' )
										),
									),
									array( 'class' => 'odd' ),
									array( 'class' => 'even' )
...
// 										&& \''.$etatdossierpcg.'\' != \'dossiertraite\'
									...
									|| '.$this->Permissions->check( 'decisionsdossierspcgs66', 'edit' ).' != \'1\'
								' )
							),
                            'Decisionsdossierspcgs66::avistechnique' => array(
                                'disabled' => ( '
									\''.$etatdossierpcg.'\' == \'transmisop\'
                                    || '.$this->Permissions->check( 'decisionsdossierspcgs66', 'avistechnique' ). ' != \'1\'
								' )
                            ),
                            'Decisionsdossierspcgs66::validation' => array(
                                'disabled' => ( '
									\''.$etatdossierpcg.'\' == \'transmisop\'
                                    || '.$this->Permissions->check( 'decisionsdossierspcgs66', 'validation' ). ' != \'1\'
								' )
                            ),
							'Decisionsdossierspcgs66::print' => array(
								...
								'disabled' => (
									( $this->Permissions->check( 'decisionsdossierspcgs66', 'print' ) != "1" )
								)
							),
							'Decisionsdossierspcgs66::transmettreop' => array(
								...
								'disabled' =>  '( "'.$this->Permissions->check( 'decisionsdossierspcgs66', 'transmitop' ).'" != "1" ) || ( "#Decisiondossierpcg66.validationproposition#" == "N" )'
							),
							'Decisionsdossierspcgs66::delete' => array(
								'label' => 'Supprimer',
								'url' => array(  'controller' => 'decisionsdossierspcgs66', 'action' => 'delete' ),
								'disabled' =>  '( "'.$this->Permissions->check( 'decisionsdossierspcgs66', 'delete' ).'" != "1" )'
							),
                            'Decisionsdossierspcgs66::filelink' => array(
								'label' => 'Fichiers liés',
								'url' => array(  'controller' => 'decisionsdossierspcgs66', 'action' => 'filelink' ),
								'disabled' =>  '( "'.$this->Permissions->check( 'decisionsdossierspcgs66', 'filelink' ).'" != "1" )'
							)
						),
						'add' => array(
							'Decisiondossierpcg66.add' => array( 'controller'=>'decisionsdossierspcgs66', 'action'=>'add', $dossierpcg66_id ),
							'disabled' => ( /*$etatdossierpcg != 'attavistech' || */$this->Permissions->check( 'decisionsdossierspcgs66', 'add' ) != "1" )
						),
						'options' => $options
					)
				)
