...
<?php echo $this->Xform->create( 'Cohortenonoriente66', array( 'type' => 'post', 'action' => 'notisemploi', 'id' => 'Search', 'class' => ( ( is_array( $this->request->data ) && !empty( $this->request->data ) ) ? 'folded' : 'unfolded' ) ) );?>
		...
		<fieldset>
			...
			<?php
				...
				echo $this->Xform->input( 'Search.Nonoriente66.dateimpression_to', Set::merge( array( 'label' => 'Au (exclus)', 'type' => 'date', 'dateFormat' => 'DMY', 'maxYear' => date( 'Y' ) + 5, 'minYear' => date( 'Y' ) - 20), $dateimpressionToSelected ) );
			?>
		</fieldset>
		<fieldset>
