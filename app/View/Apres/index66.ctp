...
<div class="with_treemenu">
	<h1><?php echo $this->pageTitle;?></h1>
			...
			<?php endif;?>
			<?php if( $this->Permissions->check( 'apres'.Configure::read( 'Apre.suffixe' ), 'add' ) ):?>
				<ul class="actionMenu">
					<?php
					...
					echo $this->Xhtml->tableCells(
						array(
							...
							$this->Xhtml->printLink(
								...
								$buttonEnabledInc
                                && $this->Permissions->check( 'apres66', 'impression' )
                                && ( Set::classicExtract( $apre, 'Apre66.etatdossierapre' ) != 'ANN' )
							),
							$this->Default2->button(
								...
								array(
									...
									'enabled' => (
                                        $buttonEnabled
                                        && $this->Permissions->check( 'apres'.Configure::read( 'Apre.suffixe' ), 'maillink' )
                                        && ( Set::classicExtract( $apre, 'Apre66.etatdossierapre' ) != 'ANN' )
                                    )
								)
							),
...
//                                && ( Set::classicExtract( $apre, 'Apre66.etatdossierapre' ) != 'ANN' )
                            ...
                            $this->Default2->button(
								...
								array(
                                    'enabled' => (
                                        $this->Permissions->check( 'apres'.Configure::read( 'Apre.suffixe' ), 'cancel' )
                                        && ( Set::classicExtract( $apre, 'Apre66.etatdossierapre' ) != 'ANN' )
