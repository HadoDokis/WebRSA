...
<script type="text/javascript">
	...
	document.observe("dom:loaded", function() {
        ...
        // Suiteau à l'ajout du bloc mail employeur
        observeDisableFieldsetOnValue(
			'CuiSendmailemployeur',
			$( 'sendmailemployeur' ),
			['1'],
			false,
			true
...
<fieldset>
    <?php
        echo $this->Observer->disableFieldsOnValue(
            'Cui.dossierrecu',
            array(
                'Cui.datedossierrecu.day',
                'Cui.datedossierrecu.month',
                'Cui.datedossierrecu.year',
                'Cui.dossiereligible'
            ),
            array( '1' ),
            false,
        ...
        echo $this->Observer->disableFieldsOnValue(
            'Cui.dossiereligible',
            array(
                'Cui.datedossiereligible.day',
                'Cui.datedossiereligible.month',
                'Cui.datedossiereligible.year',
                'Cui.dossiercomplet'
            ),
            array( '1' ),
            false,
        ...
        echo $this->Observer->disableFieldsOnValue(
            'Cui.dossiercomplet',
            array(
                'Cui.datedossiercomplet.day',
                'Cui.datedossiercomplet.month',
                'Cui.datedossiercomplet.year'
            ),
            array( '1' ),
            false,
            true
...
<!--**************************************** Partie du bloc SALARIE jusqu'au dates du CUI *********************************************** -->
<?php if( $isDossierComplet ):?>
...
<fieldset id="infoscui" class="noborder">
    <!--**************************************** Partie SALARIE *********************************************** -->
<?php
    /**
    ...
    */
    ...
    <fieldset>
        ...
        <?php if( !empty( $listeRSA ) ):?>
            <table class="aere tooltips mediumSize">
                <thead>
                    <tr>
                        <th>Nature(s) RSA</th>
                        <th>Montant(s) RSA</th>
        ...
        <table class="cui3 noborder">
            <tr>
                ...
                <td class=" noborder">
                <?php
                    // Calcul du délai entre aujourd'hui et la date de demande rsa
                    $dateFrom = new DateTime(strftime('%d-%m-%Y',time()));
                    $dateNow = new DateTime(strftime($dataCaf['Dossier']['dtdemrsa']));
                    $delaiEntreeDispositif = $dateNow->diff($dateFrom);
                    echo 'Date de demande RSA : '.$this->Locale->date( 'Date::short', Set::extract( $dataCaf, 'Dossier.dtdemrsa' ) ).' ('.$delaiEntreeDispositif->format('%y an(s) %m mois %d jours').')';
                ?>
                </td>
            </tr>
        </table>
	...
	<script type="text/javascript">
		document.observe( "dom:loaded", function() {
		 ...
		 //FIXME: A ajouter une fois le code remis en place
         observeDisableFieldsetOnValue(
			'CuiSecteurcuiId',
			$( 'iscae' ),
			['<?php echo implode( "', '", $secteur_isnonmarchand_id );?>'],
    ...
    <!-- Gestion de l'affichage des taux selon le type de CUI -->
    <script type="text/javascript">
        var taux = [];
...
<?php endif;?>
<!-- Fin du bloc devant être masqué -->
<!--**************************************** Fin du bloc SALARIE jusqu'au dates du CUI *********************************************** -->
<!-- *************************************** Bloc pour les dates ********************************************************************* -->
<fieldset>
    <table class="wide noborder">
			<tr>
