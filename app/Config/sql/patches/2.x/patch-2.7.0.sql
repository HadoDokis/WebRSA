...
$$
	...
	BEGIN
		v_select_query := 'SELECT isc.table_schema, isc.table_name, isc.column_name, isc.udt_name
		...
		LOOP
			...
			RAISE NOTICE  '%', v_altercolumn_query;
			...
			RAISE NOTICE  '%', v_inlist_query;
			EXECUTE v_inlist_query;

			-- 3°) Suppression de l'ENUM si on est le seul à l'utiliser
			IF v_row_count.count = 1 THEN
				v_droptype_query := 'DROP TYPE IF EXISTS ' || v_row_field.udt_name || ';';
				RAISE NOTICE  '%', v_droptype_query;
...
-- SELECT public.enumtype_to_validate_in_list( 'public', 'cuis', 'secteur' );
-- SELECT public.table_enumtypes_to_validate_in_list( 'public', 'cuis' );
-- TODO: 'attavismne'::type_positioncui66, ... -> valeurs par défaut lorsque le type est encore utilisé ailleurs
