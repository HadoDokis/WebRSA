	...
	class Prg2Component extends Component
	{
		...
		public function startup( &$controller ) {
			if( in_array( $controller->action, array_keys( $this->_prgActions ) ) ) { // FIXME: function avec *
				if( !empty( $controller->params['form'] ) ) { // FIXME: dans Prg, on faisait un merge avec $this->data juste avant
					return;
				}
...
//							$this->log( var_export( $this->Session->read( 'Prg' ), true ), LOG_DEBUG );
					...
					$params = Set::merge( $controller->params['named'], $params );
					...
					if( isset( $controller->params['named']['sessionKey'] ) ) {
						...
						$controller->data = Set::merge( $controller->data, $sessionParams );
