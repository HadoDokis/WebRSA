	...
	class Cohortesreferents93Controller extends AppController
	{
		...
		protected function _index( $structurereferente_id ) {
			if( !empty( $this->data ) ) {
				...
				if( ( $this->action == 'affecter' ) && isset( $this->data['PersonneReferent'] ) ) {
					$dossiers_ids = array_unique( Set::extract( '/PersonneReferent/dossier_id', $this->data ) );
					...
					$data = Set::extract( '/PersonneReferent[action]', $this->data );
					if( $this->PersonneReferent->saveAll( $data, array( 'validate' => 'only', 'atomic' => false ) ) ) {
						$this->PersonneReferent->begin();

						$data = Set::extract( '/PersonneReferent[action=Valider]', $this->data );
						...
						if( !empty( $data ) && $this->PersonneReferent->saveAll( $data, array( 'validate' => 'first', 'atomic' => false ) ) ) {
							...
							unset( $this->data['PersonneReferent'] );
						}
						else {
				...
				$personnes_referents = $this->paginate(
					...
					array(),
					array(),
					!Set::classicExtract( $this->data, 'Search.Pagination.nombre_total' )
				);
				...
				if( $this->action == 'affecter' ) {
					...
					if( !isset( $this->data['PersonneReferent'] ) ) {
						$this->data['PersonneReferent'] = array();
						foreach( array_keys( $personnes_referents ) as $index ) {
							$this->data['PersonneReferent'][$index] = array( 'action' => 'En attente' );
						}
					}
				}
			}
		...
		public function exportcsv() {
			...
			$data = Xset::bump( $this->params['named'], '__' );
			$querydata = $this->Cohortereferent93->search(
				'affectes',
				(array)$this->Session->read( 'Auth.Zonegeographique' ),
				$this->Session->read( 'Auth.User.filtre_zone_geo' ),
