...
<div class="with_treemenu">
    ...
    <?php
        ...
        echo $xhtml->tag(
            'h1',
            $this->pageTitle = __d( $domain, "Modescontact::{$this->action}", true )
        );
    ?>
<?php
...
?>
    <?php
		echo $default2->index(
			...
			array(
				'actions' => array(
					'Modescontact::view' => array( 'domain' => $domain, 'disabled' =>  '( "'.$permissions->check( 'modescontact', 'view' ).'" != "1" )' ),
					'Modescontact::edit' => array( 'domain' => $domain, 'disabled' =>  '( "'.$permissions->check( 'modescontact', 'edit' ).'" != "1" )' )
				),
