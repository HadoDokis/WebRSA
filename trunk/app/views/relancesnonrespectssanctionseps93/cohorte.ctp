...
<?php
	...
	echo $xhtml->tag( 'fieldset', $xhtml->tag( 'legend', 'Recherche par bénéficiaire' ).
		$default->subform(
			array(
				...
				'Search.Serviceinstructeur.id' => array( 'domain' => 'relancenonrespectsanctionep93', 'label' => __d( 'relancenonrespectsanctionep93', 'Serviceinstructeur.id', true ) ),
			),
			array(
				'options' => $options
			)
	...
	echo $form->input( 'Search.Relance.numrelance', array( 'legend' => 'Type de relance à réaliser', 'type' => 'radio', 'options' => array( 1 => 'Première relance', 2 => 'Confirmation passage en EP' ), 'value' => ( isset( $this->data['Search']['Relance']['numrelance'] ) ? @$this->data['Search']['Relance']['numrelance'] : 1 ) ) );
			...
			foreach( $results as $index => $result ) {
				...
				$row = Set::merge(
					...
					array(
						...
						$xform->input( "Relancenonrespectsanctionep93.{$index}.user_id", array( 'type' => 'hidden', 'value' => $session->read( 'Auth.User.id' ) ) ).
