...
<?php
// debug($actioncandidatPersonne);
        echo "<h2>Pièces déjà présentes</h2>";
        if( !empty( $actioncandidatPersonne['Fichiermodule'] ) ){
            $fichiersLies = Set::extract( $actioncandidatPersonne, 'ActioncandidatPersonne/Fichiermodule' );
