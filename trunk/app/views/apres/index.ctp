...
<div class="with_treemenu">
	...
	<?php
		...
		if( $alerteMontantAides ) {
			$msg = __dn(
				'apre',
				'Cette personne risque de bénéficier de plus de %s € d\'aides complémentaires sur l\'année en cours',
				'Cette personne risque de bénéficier de plus de %s € d\'aides complémentaires au cours des %s dernières années',
				Configure::read( 'Apre.periodeMontantMaxComplementaires' ),
				true
			);

			echo $xhtml->tag(
				'p',
				$xhtml->image( 'icons/error.png', array( 'alt' => 'Remarque' ) ).' '.sprintf( $msg, Configure::read( 'Apre.montantMaxComplementaires' ), Configure::read( 'Apre.periodeMontantMaxComplementaires' ) ),
				array( 'class' => 'error' )
			);
		}
	?>
