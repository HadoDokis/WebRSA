...
<tr>
	...
	foreach( $dossiers[$theme]['liste'] as $i => $dossierep ) {
		...
		$hiddenFields = $form->input( "Decisionsanctionep58.{$i}.id", array( 'type' => 'hidden' ) ).
						...
						$form->input( "Decisionsanctionep58.{$i}.user_id", array( 'type' => 'hidden', 'value' => $session->read( 'Auth.User.id' ) ) );
		...
		echo $xhtml->tableCells(
			array(
				...
				array(
					$form->input( "Decisionsanctionep58.{$i}.decision2", array( 'type' => 'select', 'label' => false, 'empty' => true, 'options' => @$options['Decisionsanctionep58']['decision'], 'value' => @$dossierep['Sanctionep58']['Decisionsanctionep58'][0]['decision2'] ) ),
