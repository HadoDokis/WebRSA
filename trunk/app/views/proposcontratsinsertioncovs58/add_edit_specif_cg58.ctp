	...
	<?php
		if( $this->action == 'add' ) {
			...
			if ( isset( $avenant_id ) && !empty( $avenant_id ) ) {
				echo $form->input( 'Propocontratinsertioncov58.avenant_id', array( 'type' => 'hidden', 'value' => $avenant_id ) );
			}
			echo '</div>';
		}
		else {
	...
	<fieldset>
		...
		<?php
			if ( isset( $avenant_id ) && !empty( $avenant_id ) ) {
				echo $xhtml->tag(
					'div',
					$xform->label( __d( 'contratinsertion', 'Propocontratinsertioncov58.num_contrat', true ) ).
					'Avenant',
					array(
						'class' => 'input select'
					)
				);
				echo $xform->input( 'Propocontratinsertioncov58.num_contrat', array( 'type' => 'hidden', 'value' => $tc ) );
			}
			else {
				echo $xform->input( 'Propocontratinsertioncov58.num_contrat', array( 'label' => 'Type de contrat' , 'type' => 'select', 'options' => $options['num_contrat'], 'empty' => true, 'value' => $tc ) );
			}
		?>
		...
		<?php echo $xform->input( 'Propocontratinsertioncov58.datedemande', array( 'label' => __d( 'propocontratinsertioncov58', 'Propocontratinsertioncov58.date_saisi_ci', true ), 'type' => 'hidden' ) ) ;?>
</fieldset>
