...
//<![CDATA[
    ...
    function flip( id ) {
		$('MembreepSeanceepSuppleant'+id).disabled = !$('MembreepSeanceepMembreepId'+id).checked;	
    }
//]]>
</script>
    ...
    ?>
    <?php echo $xform->create( 'MembreepSeanceep', array( 'type' => 'post', 'url' => Router::url( null, true ) ) ); ?>
            <fieldset>
                <legend>Participants à l\'EP</legend>
                <?php echo $xform->input( 'MembreepSeanceep.seanceep_id', array( 'label' => false, 'type' => 'hidden' ) ) ;?>
                <table>
                    <thead>
                        <tr>
                            <th>Nom/Prénom</th>
                            ...
                            foreach( $membres as $i => $membre ) {
                                echo $html->tableCells(
                                    array(
                                        ...
                                        $xform->checkbox(
                                        	'MembreepSeanceep.membreep_id.'.$i, 
                                        	array(
                                        		'value' => $membre['Membreep']['id'],
                                        		//'id' => 'SeanceepMembrep'.$membreparticipant['Participantcomite']['id'] ,
                                        		'checked' => isset( $membre['MembreepSeanceep']['membreep_id']), 
                                        		'class' => 'checkbox',
                                        		'onClick' => 'flip('.$i.')'
                                        		)
                                        ),
