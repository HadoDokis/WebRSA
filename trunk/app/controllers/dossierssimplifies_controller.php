<?php
	class DossierssimplifiesController extends AppController
	{
		...
		public function add() {
			...
			if( !empty( $this->data ) ) {
				if( !empty( $this->data['Dossier']['numdemrsatemp'] ) ) {
					$this->data['Dossier']['numdemrsa'] = $this->Dossier->generationNumdemrsaTemporaire();
				}
		...
		public function edit( $personne_id = null, $orient_id = null ) {
			...
			if( !empty( $this->data ) ) {
				...
				$this->data['Orientstruct'][0]['user_id'] = $this->Session->read( 'Auth.User.id' );
				...
				if( $this->Personne->saveAll( $this->data, array( 'validate' => 'only' ) ) && isset( $this->data['Orientstruct'][0]['typeorient_id'] ) && isset( $this->data['Orientstruct'][0]['structurereferente_id'] ) ) {
					...
					$this->data['Orientstruct'][0]['date_propo'] = strftime( '%Y-%m-%d', time() ); // FIXME
					$this->data['Orientstruct'][0]['date_valid'] = strftime( '%Y-%m-%d', time() ); // FIXME
				}

				$this->Dossier->begin();
				$saved = $this->Personne->saveAll( $this->data, array( 'atomic' => false ) );
