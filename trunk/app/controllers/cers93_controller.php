	...
	class Cers93Controller extends AppController
	{
		...
		public function edit( $id = null ) {
			...
			if( !empty( $this->data ) ) {
				...
				if( $this->Cer93->Contratinsertion->saveAll( $this->data, array( 'validate' => 'first', 'atomic' => false ) ) ) {
					$this->Cer93->Contratinsertion->commit();
					$this->Jetons2->release( $dossier_id );
					$this->Session->setFlash( 'Enregistrement effectué', 'flash/success' );
//					$this->redirect( array( 'action' => 'index', $this->data['Contratinsertion']['personne_id'] ) );
				}
				else {
				...
				$this->data = $this->Cer93->Contratinsertion->find(
					'first',
					array(
						'fields' => array_merge(
