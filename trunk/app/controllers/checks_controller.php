	...
	class ChecksController extends AppController
	{
		...
		protected function _checkModelesOdt() {
			...
			foreach( Configure::listObjects( 'model' ) as $modelName ) {
				if( !preg_match( '/([0-9]{2})$/', $modelName, $matches ) || ( $matches[1] == Configure::read( 'Cg.departement' ) ) ) {
					...
					if( !empty( $modelesOdtVar ) ) {
						...
						$modeles = array_merge(
								$modeles, array_words_replace(
										$modelesOdtVar, array( '%s' => $alias )
								)
						);
					}
				}
			...
			$modeles['errors'] = Set::merge(
							$modelesOdtStatiques, $modelesOdtVariables, $modeles['errors']
			);
...
// debug($return);
		...
		protected function _checkFiltresdefaut() {
			$errors = array();
			$paths = array();

			if( Configure::read( 'Cg.departement' ) == 93 ) {
				$paths = array(
					'Filtresdefaut.Cohortes_nouvelles',
					'Filtresdefaut.Cohortes_enattente',
