			...
			for ($i=0; $i<$obj->runs; $i++) {
				...
				foreach ($fields as $fieldName => $params) {
					if (!in_array($fieldName, $fieldsName)) {
						debug(array('Champs disponnibles :' => $fieldsName));
						trigger_error("Le champ ".$fieldName." n'existe pas dans ".$obj->modelName." !");
					}
				...
				if ($saveInDb && !$success) {
					debug(
						array(
							...
							'Contenu des données' => $save
						)
					);
					trigger_error("L'enregistrement sur ".$obj->modelName." a échoué !");
				}
