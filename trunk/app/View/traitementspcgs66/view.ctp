<?php
	$this->pageTitle =  __d( 'traitementpcg66', "Traitementspcgs66::{$this->action}", true );
...
<div class="with_treemenu">
	<?php
		...
		echo $form->create( 'Traitementpcg66', array( 'type' => 'post', 'id' => 'traitementpcg66form', 'url' => Router::url( null, true ) ) );
		echo $default2->view(
			$traitementpcg66,
			array(
				'Descriptionpdo.name', 
			...
			echo '<table><tbody>';
				...
				if( isset( $courriersLies ) ){
					foreach( $courriersLies as $i => $courriers ){
						...
						echo '<td>'.$xhtml->link( 'Imprimer', array( 'action' => 'printCourrier', $courriers['Courrierpdo']['CourrierpdoTraitementpcg66']['id']    ) ).'</td></tr>';
					}
				}
			echo '</tbody></table>';
		}
