<?php
	...
	$csv->addRow( array( 'N° dossier RSA', 'Nom allocataire', 'Prénom allocataire', 'Date de naissance', 'Commune de l\'allocataire', 'Date de validation de l\'orientation', 'Nb de jours depuis la fin du contrat lié', 'Type d\'orientation', 'Structure référente', 'Référent' ) );
	...
	foreach( $orientsstructs as $orientstruct ) {
		...
		$row = array(
			...
			$locale->date( 'Date::short', Set::classicExtract( $orientstruct, 'Orientstruct.date_valid' ) ),
