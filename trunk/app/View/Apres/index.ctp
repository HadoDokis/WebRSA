...
<div class="with_treemenu">
    <h1><?php echo $this->pageTitle;?></h1>
            ...
            <?php if( $this->Permissions->check( 'apres'.Configure::read( 'Apre.suffixe' ), 'add' ) /*&& ( $apre_forfait > 0 )*/ ):?>
                <ul class="actionMenu">
                    <?php
...
//                                     $this->Permissions->check( 'relancesapres', 'view' )
//                                 ),
                                $this->Xhtml->editLink(
                                    'Editer la relance',
                                    array( 'controller' => 'relancesapres', 'action' => 'edit', Set::classicExtract( $relanceapre, 'Relanceapre.id' ) ),
                                    ( $this->Permissions->check( 'relancesapres', 'edit' ) && Set::classicExtract( $relanceapre, 'Relanceapre.id' ) == $lastrelance_id  )
                                ),
                                $this->Xhtml->printLink(
                                    ...
                                    $this->Permissions->check( 'gedooos', 'relanceapre' )
                                )
                            ),
                            array( 'class' => 'odd' ),
                            array( 'class' => 'even' )
