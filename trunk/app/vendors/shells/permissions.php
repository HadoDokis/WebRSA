    ...
    class PermissionsShell extends AppShell
    {
		...
		*/
		...
		public function initialize() {
			...
			$this->Acl->startup( $this->Controller );
		...
		*/
		...
		protected function _clean() {
			$success = $this->Acl->Aro->deleteAll( array( '1 = 1' ) )
				&& $this->Acl->Aco->deleteAll( array( '1 = 1' ) )
				&& $this->Acl->Aro->Permission->deleteAll( array( '1 = 1' ) );
			...
			foreach( array( 'aros', 'acos', 'aros_acos' ) as $table ) {
				...
				$success = $this->Acl->Aro->query( $sql ) && $success;
			}
		...
		*/
		...
		protected function _addUsers() {
			...
			foreach( $users as $id => $user ) {
				$userAro = $this->Acl->Aro->findByAlias( $user['User']['username'] );
				$groupAro = $this->Acl->Aro->findByAlias( $user['Group']['name'] );
